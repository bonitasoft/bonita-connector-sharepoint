...
public class ConnectorTestToolkit {
    ...
    /**
     * Reads a single output parameter from a connector using reflection.
     * This is needed because {@code getOutputParameters()} has protected access
     * in {@code AbstractConnector} and test classes are in a different package.
     */
    @SuppressWarnings("unchecked")
    public static Object getOutput(AbstractSharePointConnector connector, String name) throws Exception {
        var method = org.bonitasoft.engine.connector.AbstractConnector.class.getDeclaredMethod("getOutputParameters");
        method.setAccessible(true);
        var outputs = (java.util.Map<String, Object>) method.invoke(connector);
        return outputs.get(name);
    }

    /**
     * Reads all output parameters from a connector using reflection.
     */
    @SuppressWarnings("unchecked")
    public static java.util.Map<String, Object> getOutputs(AbstractSharePointConnector connector) throws Exception {
        var method = org.bonitasoft.engine.connector.AbstractConnector.class.getDeclaredMethod("getOutputParameters");
        method.setAccessible(true);
        return (java.util.Map<String, Object>) method.invoke(connector);
    }
